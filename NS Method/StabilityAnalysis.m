...
% URMin = 1; %Min reduced speed
% URMax = 3; % Max reduced speed
% steps = 8; % Number of steps in the speed continuation method
% UR = URMin:(URMax-URMin)/steps:URMax;
% UR = [2,4,5,6,7,7.5,8,8.2,8.4,8.6,8.8,9,9.2,9.4,9.6,9.8,10,11,12,14];
UR = 18;
E = (12*(1-nus^2)*rhos*l^2*U^2)./(hp^2.*(UR.^2)); %Plate Young's modulus
D = E.*(hp^3/(12*(1-nus^2)));
...
%% Compute base flow for given Reynold
...
[A,B,As,Bs,sizeFu,sizeFp,sizeFl,sizeSu,sizeSe, FixedFu, FixedFp, FixedFl, FixedSu, FixedSe] = runCoupledP2P1(nH,Re,E(1),nus,rhos); 

% Matrix balancing method that can help eigs converge
% [T, Ab] = spbalance(A);
% Bb = T\B*T;

% Options that can be used to help eigs converge
% opts.tol = 1e-20;
% opts.maxit = 500;

[EigVectTemp,EigValTemp] = eigs(A,-1.0i.*B,50,0);
% [EigVectTemp,EigValTemp] = eigs(Ab,Bb,50,sigma,opts);
EigValTemp = diag(EigValTemp);

% Display Argand diagram of the found solutions
figure(); hold on;
...
f = waitbar(0,'Continuation on UR : Starting');

for i = 2:length(UR)
    
    x = UR(i)-UR(1)/(UR(length(UR)));
    waitbar(x,f,strcat('Continuation on UR : ',num2str(UR(i))));

    [Atemp,Btemp,As,Bs,sizeFu,sizeFp,sizeFl,sizeSu,sizeSe, FixedFu, FixedFp, FixedFl, FixedSu, FixedSe] = runCoupledP2P1(nH,Re,E(1),nus,rhos); 
    ...
    divMax = 4; % Number of step division allowed
    divNum = 0;
    
    while MacMin < MacCrit || UR_temp > UR(i)

        if divNum <= divMax 
            x = UR_temp-UR(1)/(UR(length(UR)));
            waitbar(x,f,strcat('Continuation on Re : ',num2str(UR_temp),' - MAC: ',num2str(MacMin,'%.3f')));
...
% Stuff for testing, visualization and old versions
...
for i = 1:length(Re)
    ...
    for j = 2:length(E)
        ...
        while MacMin < MacCrit || E_temp > E(j)
            ...
            x = ((URTemp-URMin)/(URMax-URMin)/length(Re))+((i-1)/length(Re));
            waitbar(x,f,strcat('Continuation on UR : ',num2str(URTemp),' - Re : ',num2str(Re(i)),' - MAC: ',num2str(MacMin,'%.3f')));
            ...
            for k = 1:length(freq)
                ...
                if j == 2
                    ...
                    EigValGuess = ((EigValPast(k)-EigValPast2(k))/(URPast-URPast2))*(URTemp-URPast)+EigValPast(k);
                end
...
%         plot(UR,real(EigVal_bar),'-');
%         plot(UR,-imag(EigVal_bar),'.');
%         
%     end
% 
...
%     else
%         EigGuess = ((EigVal1(i-1)-EigVal1(i-2))/(UR(i-1)-UR(i-2)))*(UR(i)-UR(i-1))+EigVal1(i-1);
%     end
%     
...
% %%
% i =  9;
% dispEigVect(EigVect1(:,i),sizeS,sizeFv,sizeFp, FixedS, FixedFv,FixedFp,strcat("Mode 1 - ",num2str(EigVal1(i))," - U: ",num2str(UR(i))),2);
% 
