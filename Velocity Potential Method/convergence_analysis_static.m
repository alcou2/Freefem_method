...
for i = 1:length(hp)
    [eV(:,i), sizeS(i), sizeF(i), FixedS, FixedF] = runCoupled_CA(U,mhS,mhF,rho_f,hp(i),rho_s(i),RL1,RL2,50);
    EigFreq(:,i) = eV(:,i);
...
plot(hp/l,real(EigFreq_bar(1,:)./omega_ref_1),'k');
plot(hp/l,real(EigFreq_bar(3,:)./omega_ref_2),'b');
...
hold on;
xlabel('$n_{hf}$');
ylabel('$\omega/\omega_ref$');

omega_ref_1 = 2.773;
omega_ref_2 = 18.204;

plot(mhF,real(EigFreq_bar(1,:)./omega_ref_1),'k');
plot(mhF,real(EigFreq_bar(3,:)./omega_ref_2),'b');
